        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and not started:
                    started = True
                    for row in grid:
                        for spot in row:   # for every spot-obj in gridsp
                            spot.update_neighbors(grid)  # call update_neighbors method on spot-obj pass grid
                ...
                for w in row:
                    if w.energy <= 0:
                        ...
                        if w.reproduce() == True  and rand.randint(0,100) <= 50:
                            wolfies[i].append(Wolf(rand.randint(0,ROWS-1), rand.randint(0,ROWS-1), GAP, ROWS))
                ...
                for s in row:
                    if s.energy <= 0:
                        ...
                        if s.reproduce() == True and rand.randint(0,100) <= 50:
                            sheepies[i].append(Sheep(rand.randint(0,ROWS-1), rand.randint(0,ROWS-1), GAP, ROWS))
                        
 
            for i, row in enumerate(grid):    # CHECK SHEEP-GRASS COLLISION
                for g in row:
                    ...
                    for rowW in sheepies:
                        for s in rowW:
                            if g.energy >= 100:
                                ...
                                g.energy = 0
                            if g.color == BROWN:
                                g.energy += g.regrowth_rate
                            
                            if g.row == s.col and g.col == s.row and g.color == BROWN:
                                if g in grid[i]:
                                    pass
                                    #s.energy -= 10
                            
                            if g.row == s.col and g.col == s.row and g.color == GREEN:
                                if g in grid[i]:
                                    g.color = BROWN # comment this line to give sheep infite food
                                    g.energy = 0
                                    s.energy += 10   # uncomment this line to give sheep infite grass
